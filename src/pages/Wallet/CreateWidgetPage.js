...
import { Row, Col, Input, Form, Select, Radio, Skeleton, message } from "antd";
...
const CreateWidgetPage = () => {
  ...
  const onFinish = (values) => {
    ...
    CreateNewWidget(request_data, walletid)
      .then(() => {
        setIsSubmitting(false);
        history.push("/wallet");
        message.success("Create widget successful!");
      })
      .catch(() => {
        message.error("Error when create new widget. Please try again");
        setIsSubmitting(false);
      });
  };
  ...
  useEffect(() => {
    GetSingleWalletDetail(walletid)
      .then((res) => {
